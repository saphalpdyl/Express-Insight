...
const logger = function(settings) {
  ...
  let logStream = fs.createWriteStream(path.join(monthYearLogFolderPath,`${(new Date()).toLocaleDateString().split("/").join("_")}.log`), {
    flags: "a",
  });
  ...
  return function(req, res, next) {
    ...
    if ( monthYearString != getSystemMonthYear() ) {
      ...
      logStream = fs.createWriteStream(path.join(monthYearLogFolderPath,`${(new Date()).toLocaleDateString().split("/").join("_")}.log`), {
        flags: "a",
      });
    }
    ...
    // Checking for and creating a sqlite3 database if needed
    if (!checkIfFileExists(path.join(logFolderPath, "log.sqlite3")))
      initializeDB(logFolderPath, config.dbFileName);
